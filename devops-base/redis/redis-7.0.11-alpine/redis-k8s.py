...
def update_cluster_config():
    ...
    if not os.path.exists(cluster_file) or (lines and len(lines.split('\n'))<4):
        ...
        print("当前副本数是: ", current_replicas)
        if current_replicas == None:
        ...
        if current_replicas==replicas:
            ...
            time.sleep(5)
            rs=os.popen("echo 'yes'|redis-cli --cluster create --cluster-replicas 0 -a %s --no-auth-warning %s"%(redis_passwd,(' ').join(cluster_instances)))
            res=rs.read()
